...
const handleSubmit = async (event) => {
  ...
  try {
    const response = await fetch(`${process.env.NEXT_PUBLIC_APP_URL}/hospitals/hubs/subhubs/clusters`, {
      method: "POST",
      headers: {
...
useEffect(() => {
  ...
  useEffect(() => {
    const fetchHospitals = async () => {
      try {
        const token = Cookies.get('authToken');
        const response = await axios.get(`${process.env.NEXT_PUBLIC_APP_URL}/hospitals/hubs/subhubs/clusters?hospitalId=${hospitalId}`, {
          headers: {
            Authorization: `Bearer ${token}`, 
          },
        });
...
// Fetch states from API
  ...
  <h3>CLUSTERS: {hospitalName}</h3>

  <div style={{ display: "flex", gap: "10px" }}>
    <Button onClick={() => router.push(`/dashboard/nicrat-ict/subhubs?hospitalId=${hospitalId}&hospital=${hospitalName}`)}>
      Back
    </Button>
          ...
          <TableBody>
            {displayedHospitals.map((hospital) => (
              <TableRow key={hospital.hospitalId}>
                ...
                <TableCell>
 <Button onClick={() => router.push(`/dashboard/nicrat-ict/subhubs?hospital=${hospital?.hospital?.hospitalName}&hospitalId=${hospitalId}`)}>
          Clusters
        </Button>
</TableCell>
...
export default NICRATClustersTable;
