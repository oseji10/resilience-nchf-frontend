...
const NICRATPendingPatientsTable = () => {
  ...
  useEffect(() => {
    const fetchPatients = async () => {
      try {
        ...
        const response = await axios.get(`${process.env.NEXT_PUBLIC_APP_URL}/patient/nicrat/pending`, {
  ...
  const handleAssignCarePlan = async () => {
    ...
    try {
      ...
      const response = await axios.post(
        `${process.env.NEXT_PUBLIC_APP_URL}/patient/nicrat/assessment`,
        {
  ...
  return (
    <Box>
      <Typography variant="h6" gutterBottom>
        Patients Pending DG's Approval
      </Typography>
      <TextField
  ...
  <Box sx={{ display: 'inline-block', px: 1, py: 0.5, borderRadius: 1, backgroundColor: 'error.light', color: 'error.contrastText' }}>
    PENDING APPROVAL
       ...
       <Dialog open={openAssignDoctorModal} onClose={handleCloseAssignDoctorModal}>
              <DialogTitle>Patient Approval</DialogTitle>
              <DialogContent>
...
export default NICRATPendingPatientsTable;
